...
img_apse2.name
img.name
ami_name = 'amzn2-ami-hvm-2.0.202009*-x86_64-gp2'
filters = [
    {
        'Name': 'name',
        'Values': [ami_name]},
...
list(ec2.images.filter(Owners=['amazon'], Filters=filters).limit(count=1))
list(ec2_apse2.images.filter(Owners=['amazon'], Filters=filters).limit(count=1))
img
key
